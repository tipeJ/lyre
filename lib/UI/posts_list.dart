...
class PostsListState extends State<PostsList> with TickerProviderStateMixin, PreviewCallback {
  ...
  bool isVidControlsElevated = false;
  ...
  double maxVidControlsHeight = 150;
  ...
  double videoControlsLerp(double min, double max) => lerpDouble(min, max, _videoControlsController.value);
  
  void _handleVideoDragUpdate(DragUpdateDetails details) {
    _videoControlsController.value -= details.primaryDelta /
        maxVidControlsHeight; //<-- Update the _videoControlsController.value by the movement done by user.
  }

  void _reverseVideoControls() {
    _videoControlsController.fling(velocity: -2.0);
    isVidControlsElevated = false;
  }

  void _handleVidDragEnd(DragEndDetails details) {
    if (_videoControlsController.status == AnimationStatus.completed) {
      isVidControlsElevated = true;
    }
    if (_videoControlsController.isAnimating ||
        _videoControlsController.status == AnimationStatus.completed) return;

    final double flingVelocity = details.velocity.pixelsPerSecond.dy /
        maxVidControlsHeight; //<-- calculate the velocity of the gesture
    if (flingVelocity < 0.0) {
      _videoControlsController.fling(
          velocity: max(2.0, -flingVelocity)); //<-- either continue it upwards
      isVidControlsElevated = true;
    } else if (flingVelocity > 0.0) {
      _videoControlsController.fling(
      ...
      _videoControlsController.fling(
          velocity: _videoControlsController.value < 0.5
              ? -2.0
              : 2.0); //<-- or just continue to whichever edge is closer
  }
              ...
              return new AnimatedOpacity(
                ...
                child: Container(
                  ...
                  child: new Column(
                    ...
                    children: <Widget>[
                      new Container(
                        height: navBarLerp(45.0, maxNavBarHeight) + paramsHeight,
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              ...
                              new Container(
                                ...
                                height: navBarLerp(0, maxNavBarHeight-50.0),
