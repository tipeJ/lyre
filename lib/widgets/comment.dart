...
class CommentWidget extends StatefulWidget {
  ...
  const CommentWidget(this.comment, this.location, this.previewSource,
...
class _CommentWidgetState extends State<CommentWidget> {
              ...
              items: <ActionItems>[
                ActionItems(
                    icon: Icon(
                      ...
                      color: widget.comment.vote == VoteState.upvoted
                          ? Colors.amber
                          : Colors.grey,
                    ),
                    onPress: () async {
                ...
                ActionItems(
                    icon: Icon(
                      ...
                      color: widget.comment.vote == VoteState.downvoted
                          ? Colors.purple
                          : Colors.grey,
                    ),
                    onPress: () async {
                      await changeVoteState(
                          VoteState.downvoted, widget.comment);
                      setState(() {});
                    }),
                ActionItems(
                    icon: Icon(
                ...
                ActionItems(
                    ...
                    onPress: () {
                      CommentOptionsNotification(comment: widget.comment)
                        ..dispatch(context);
                    }),
              ],
              child: Column(
                ...
                children: [
                                  ...
                                  child: Text.rich(
                                    TextSpan(
                                        ...
                                        children: [
                                          ...
                                          TextSpan(
                                              text:
                                                  "${widget.comment.subreddit.displayName}",
                                              style: TextStyle(
                  ...
                  new Padding(
                      child: MarkdownBody(
                        ...
                        onTapLink: (str) => handleLinkClick(str, context),
                      ),
                      padding: const EdgeInsets.only(
                          left: _contentEdgePadding,
                          right: 16.0,
                          top: 6.0,
                          bottom: 12.0)),
                ],
          ...
          StatefulBuilder(
            builder: (BuildContext context, setState) {
              return prefix1.Visibility(
                  child: _replyWidget(), visible: _replyVisible);
            },
          ),
          Container(
                    ...
                    if (returnValue is Comment) {
                      setState(() {
                        ...
                        BlocProvider.of<CommentsBloc>(context).add(AddComment(
                            location: widget.location, comment: returnValue));
                      });
                    }
                  });
                },
  ...
  void _send() {
    ...
    if (_replyController.text.isEmpty) {
      final textSnackBar = SnackBar(
        content: Text("Cannot reply with an empty message"),
      );
    ...
    reply(widget.comment, _replyController.text).then((value) {
      ...
      if (value is String) {
        final textSnackBar =
            SnackBar(content: Text("Error sending comment: $value"));
        Scaffold.of(context).showSnackBar(textSnackBar);
        setState(() {
          _replySendingState = SendingState.Inactive;
        });
        ...
        setState(() {
          ...
          BlocProvider.of<CommentsBloc>(context)
              .add(AddComment(location: widget.location, comment: value));
        });
      }
    });
  }
...
List<Widget> _commentContentChildren(
    BuildContext context, Comment comment, PreviewSource previewSource) {
  return [
    new Padding(
        child: Row(
          ...
          children: <Widget>[
            Text(
              ...
              style: Theme.of(context)
                  .textTheme
                  .body2
                  .apply(color: getScoreColor(comment.vote, context)),
            ),
            Text(
              "● u/${comment.author}",
              style: Theme.of(context).textTheme.body2,
                    ...
                    child: Text.rich(
                      TextSpan(
                          ...
                          children: [
                            ...
                            TextSpan(
                                text: "${comment.subreddit.displayName}",
                                style: TextStyle(
                                    color: Theme.of(context)
                                        .textTheme
                                        .body1
...
class CommentContent extends StatelessWidget {
  final Comment comment;
  final PreviewSource previewSource;
  const CommentContent(this.comment, this.previewSource, {Key key})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: dividersWrapper(
          ...
          child: InkWell(
              onTap: () {
                if (widget.moreComments.id !=
                    BlocProvider.of<CommentsBloc>(context).loadingMoreId) {
                  setState(() {
                    BlocProvider.of<CommentsBloc>(context).loadingMoreId =
                        widget.moreComments.id;
                    BlocProvider.of<CommentsBloc>(context).add(
                        FetchMoreComments(
                            moreComments: widget.moreComments,
